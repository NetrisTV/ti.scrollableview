...
public class InkPageIndicator extends View implements ViewPager.OnPageChangeListener, View.OnAttachStateChangeListener
{
								   ...
								   float dotRevealFraction)
	{
		...
		if ((joiningFraction == 0f || joiningFraction == INVALID_FRACTION) && dotRevealFraction == 0f
	...
	private ValueAnimator createMoveSelectedAnimator(final float moveTo, int was, int now, int steps)
	{
		...
		retreatAnimation = new PendingRetreatAnimator(
			was, now, steps,
	...
	public class PendingRetreatAnimator extends PendingStartAnimator
	{
		...
		public PendingRetreatAnimator(int was, int now, int steps, StartPredicate predicate)
		{
			...
			final float initialX1 =
				now > was ? Math.min(dotCenterX[was], selectedDotX) - dotRadius : dotCenterX[now] - dotRadius;
			final float finalX1 = now > was ? dotCenterX[now] - dotRadius : dotCenterX[now] - dotRadius;
			final float initialX2 =
				now > was ? dotCenterX[now] + dotRadius : Math.max(dotCenterX[was], selectedDotX) + dotRadius;
